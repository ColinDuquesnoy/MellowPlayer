...
#define LYRICS_ENGINE_URL                                                      \
  "http://api.chartlyrics.com/apiv1.asmx/SearchLyricDirect?artist=%1&song=%2"
...
LyricsController::LyricsController(MainWindow *parent)
  ...
  connect(m_mainWindow->ui()->actionShow_lyrics, &QAction::toggled, this,
          &LyricsController::showLyricsPane);
  connect(m_mainWindow->ui()->stackedWidget, &QStackedWidget::currentChanged,
          this, &LyricsController::onPageChanged);
  connect(m_mainWindow->player(), &PlayerController::songChanged, this,
          &LyricsController::onSongChanged);
  connect(m_nam, &QNetworkAccessManager::finished, this,
          &LyricsController::onLyricsLoaded);
  connect(m_mainWindow->ui()->dockWidgetLyrics, &QDockWidget::visibilityChanged,
          this, &LyricsController::showLyricsPane);
...
void LyricsController::saveState() {
  QSettings().setValue("interface/lyricsPaneVisible",
                       m_mainWindow->ui()->dockWidgetLyrics->isVisible());
}
...
void LyricsController::onPageChanged(int page) {
  if (page == PAGE_WEB) {
    ...
    bool autoShow = QSettings()
                        .value("interface/automaticallyShowLyricsPane", false)
                        .toBool();
    bool lyricsPaneVisible =
        QSettings().value("interface/lyricsPaneVisible", false).toBool();
    if (!autoShow) {
...
void LyricsController::onSongChanged(const SongInfo &songInfo) {
  ...
  QRegExp frenchStopWords(
      "\\b(alors|au|aucuns|aussi|autre|avant|avec|avoir|bon|car|ce|cela|ces|"
      "ceux|chaque|ci|comme|comment|dans|de|des|du"
      "|dedans|dehors|depuis|devrait|doit|donc|dos|début|elle|elles|en|encore|"
      "essai|est|et|eu|fait|faites|fois|font|"
      "hors|ici|il|ils|je|juste|la|là|le|les|leur|là|ma|maintenant|mais|mes|"
      "mine|moins|mon|mot|même|ni|nommés|notre|nous"
      "|ou|où|par|parce|pas|peut|peu|plupart|pour|pourquoi|chanson|que|quel|"
      "quelle|quelles|quels|qui|sa|sans|ses|"
      "seulement|si|sien|son|sont|sous|soyez|sujet|sur|ta|tandis|tellement|"
      "tels|tes|ton|tous|tout|trop|très|tu|"
      "voient|vont|votre|vous|vu|ça|étaient|état|étions|été|être|je|suis|"
      "mauvais|mauvaise|l'|d'|ne|n'|à|un|une)\\b");
  song = song.replace(frenchStopWords, " ");

  // spanish stop words (from http://www.ranks.nl/stopwords/spanish)
  QRegExp spanishStopWord(
      "\\b(un|una|unas|unos|uno|sobre|todo|también|tras|otro|algún|alguno|"
      "alguna|algunos|algunas|ser|es|soy|eres|"
      "somos|sois|estoy|esta|estamos|estais|estan|como|en|para|atras|porque|"
      "por|qué|estado|estaba|ante|antes|siendo|"
      "ambos|pero|por|poder|puede|puedo|podemos|podeis|pueden|fui|fue|fuimos|"
      "fueron|hacer|hago|hace|hacemos|haceis|"
      "hacen|cada|fin|incluso|primero|desde|conseguir|consigo|consigue|"
      "consigues|conseguimos|consiguen|ir|voy|va|"
      "vamos|vais|van|vaya|gueno|ha|tener|tengo|tiene|tenemos|teneis|tienen|el|"
      "la|lo|las|los|su|aqui|mio|tuyo|ellos|"
      "ellas|nos|nosotros|vosotros|vosotras|si|dentro|solo|solamente|saber|"
      "sabes|sabe|sabemos|sabeis|saben|ultimo|"
      "largo|bastante|haces|muchos|aquellos|aquellas|sus|entonces|tiempo|"
      "verdad|verdadero|verdadera|cierto|ciertos|"
      "cierta|ciertas|intentar|intento|intenta|intentas|intentamos|intentais|"
      "intentan|dos|bajo|arriba|encima|usar|"
      "uso|usas|usa|usamos|usais|usan|emplear|empleo|empleas|emplean|ampleamos|"
      "empleais|valor|muy|era|eras|eramos|"
...
void LyricsController::onLyricsLoaded(QNetworkReply *reply) {
  ...
  m_mainWindow->ui()->textEditLyrics->setText(
      "<p>" + lyrics.replace("\n", "<br>") + "</p>");
  m_mainWindow->ui()->textEditLyrics->setAlignment(Qt::AlignCenter);
  if (autoShow) {
