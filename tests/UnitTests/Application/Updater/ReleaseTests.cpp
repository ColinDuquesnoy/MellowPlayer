...
SCENARIO( "releases can be compared") {
    GIVEN("a valid current release, a valid fake newer release and a valid fake older release") {
        const Release& currentRelease = Release::current();
        ...
        WHEN("newer release is compared to current release") {
            ...
            THEN("not isInferior") { REQUIRE(!isInferior); }
            THEN("not isInferiorOrEqual") { REQUIRE(!isInferiorOrEqual); }
            THEN("isGreater") { REQUIRE(isGreater); }
            THEN("isGreaterOrEqual") { REQUIRE(isGreaterOrEqual); }
            THEN("not isEqual") { REQUIRE(!isEqual); }
            THEN("isDifferent") { REQUIRE(isDifferent); }
        }
        ...
        WHEN("older release is compared to current release") {
            ...
            THEN("isInferior") { REQUIRE(isInferior); }
            THEN("isInferiorOrEqual") { REQUIRE(isInferiorOrEqual); }
            THEN("not isGreater") { REQUIRE(!isGreater); }
            THEN("not isGreaterOrEqual") { REQUIRE(!isGreaterOrEqual); }
            THEN("not isEqual") { REQUIRE(!isEqual); }
            THEN("isDifferent") { REQUIRE(isDifferent); }
        }
        ...
        WHEN("same releases are compared") {
            ...
            THEN("not isGreater") { REQUIRE(!isGreater); }
            THEN("isGreaterOrEqual") { REQUIRE(isGreaterOrEqual); }
            THEN("isEqual") { REQUIRE(isEqual); }
            THEN("not isDifferent") { REQUIRE(!isDifferent); }
