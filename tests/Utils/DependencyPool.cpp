...
StreamingServicesController& DependencyPool::getStreamingServicesController() {
    ...
    if (pStreamingServicesController == nullptr) {
        pStreamingServicesController = make_unique<StreamingServicesController>(streamingServiceLoaderMock.get(),
                                                                                streamingServiceWatcherMock.get());
...
ListeningHistory& DependencyPool::getListeningHistory() {
    if (pListeningHistory == nullptr)
        pListeningHistory = make_unique<ListeningHistory>(*dataProvider, getCurrentPlayer(),
                                                          getWorkDispatcher(), getSettings());
...
ThemeViewModel& DependencyPool::getThemeViewModel() {
    ...
    if (pThemeViewModel == nullptr)
        pThemeViewModel = make_unique<ThemeViewModel>(getStreamingServicesController(),
                                                      getSettings(), mock.get());
...
MainWindowViewModel& DependencyPool::getMainWindowViewModel() {
    if (pMainWindowViewModel == nullptr) {
        ...
        pMainWindowViewModel = make_unique<MainWindowViewModel>(
                streamingServices, listeningHistory, theme, updater, qtApp, currentPlayer, settings);
    }
...
Notifier& DependencyPool::getNotifier() {
    if (pNotifier == nullptr)
        pNotifier = make_unique<Notifier>(getCurrentPlayer(),
                                          getLocalAlbumArt(),
